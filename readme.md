...
A list of useful resources for web developers and designers. Use  `CMD + F`  to search for things like color, payments, etc. I’ve tagged different sections with keywords to make searching easier. Feel free to send me more resources to add by emailing me (kash@kg.dev) or by making a pull request.
...
- [ExtendsClass](https://extendsclass.com/) - Toolbox for developers: clients (REST, SOAP), testers (Regex, XPath, JSONPath), code checkers (PHP, Python, JS), SQLite browser, compare tools (XML, JSON, CSV) and converters (JSON, CSV, XML, Excel).
